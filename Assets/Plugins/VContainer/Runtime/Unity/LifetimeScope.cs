...
namespace VContainer.Plagins.VContainer.Runtime.Unity
{
    ...
    public partial class LifetimeScope : MonoBehaviour, IDisposable
    {
        ...
        {
            public ParentOverrideScope(Plagins.VContainer.Runtime.Unity.LifetimeScope nextParent)
            {
                lock (SyncRoot)
                {
        ...
        static readonly Stack<Plagins.VContainer.Runtime.Unity.LifetimeScope> GlobalOverrideParents = new Stack<Plagins.VContainer.Runtime.Unity.LifetimeScope>();
        static readonly Stack<IInstaller> GlobalExtraInstallers = new Stack<IInstaller>();
        static readonly object SyncRoot = new object();

        static Plagins.VContainer.Runtime.Unity.LifetimeScope Create(IInstaller installer = null)
        {
            var gameObject = new GameObject("LifetimeScope");
            gameObject.SetActive(false);
            var newScope = gameObject.AddComponent<Plagins.VContainer.Runtime.Unity.LifetimeScope>();
            if (installer != null)
            {
        ...
        public static Plagins.VContainer.Runtime.Unity.LifetimeScope Create(Action<IContainerBuilder> configuration)
            => Create(new ActionInstaller(configuration));

        public static ParentOverrideScope EnqueueParent(Plagins.VContainer.Runtime.Unity.LifetimeScope parent)
        ...
        public static ParentOverrideScope PushParent(Plagins.VContainer.Runtime.Unity.LifetimeScope parent) => new ParentOverrideScope(parent);
        ...
        public static Plagins.VContainer.Runtime.Unity.LifetimeScope Find<T>(Scene scene) where T : Plagins.VContainer.Runtime.Unity.LifetimeScope => Find(typeof(T), scene);
        public static Plagins.VContainer.Runtime.Unity.LifetimeScope Find<T>() where T : Plagins.VContainer.Runtime.Unity.LifetimeScope => Find(typeof(T));

        static Plagins.VContainer.Runtime.Unity.LifetimeScope Find(Type type, Scene scene)
        {
            using (ListPool<GameObject>.Get(out var buffer))
            {
                ...
                {
                    var found = gameObject.GetComponentInChildren(type) as Plagins.VContainer.Runtime.Unity.LifetimeScope;
                    if (found != null)
                        return found;
                }
            }
        ...
        static Plagins.VContainer.Runtime.Unity.LifetimeScope Find(Type type)
        {
#if UNITY_2022_1_OR_NEWER
            return (LifetimeScope)FindAnyObjectByType(type);
#else
            return (Plagins.VContainer.Runtime.Unity.LifetimeScope)FindObjectOfType(type);
#endif
        }

        public IObjectResolver Container { get; private set; }
        public Plagins.VContainer.Runtime.Unity.LifetimeScope Parent { get; private set; }
        ...
        public TScope CreateChild<TScope>(IInstaller installer = null)
            where TScope : Plagins.VContainer.Runtime.Unity.LifetimeScope
        {
        ...
        public Plagins.VContainer.Runtime.Unity.LifetimeScope CreateChild(IInstaller installer = null)
            => CreateChild<Plagins.VContainer.Runtime.Unity.LifetimeScope>(installer);
        ...
        public Plagins.VContainer.Runtime.Unity.LifetimeScope CreateChild(Action<IContainerBuilder> installation)
            => CreateChild<Plagins.VContainer.Runtime.Unity.LifetimeScope>(new ActionInstaller(installation));
        ...
        void InstallTo(IContainerBuilder builder)
        {
            ...
            builder.RegisterInstance<Plagins.VContainer.Runtime.Unity.LifetimeScope>(this).AsSelf();
            EntryPointsBuilder.EnsureDispatcherRegistered(builder);
        }

        Plagins.VContainer.Runtime.Unity.LifetimeScope GetRuntimeParent()
        {
